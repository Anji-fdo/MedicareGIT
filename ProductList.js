...
export default function ProductList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:8070/drug/');
                ...
                <div className="row">
                    {filteredDrugs.map((drug) => (
                        <div key={drug._id} className="card border-0 rounded-0 shadow mx-1 col-2">
                            <img src={`http://localhost:8070/${drug.file.replace(/\\/g, '/')}`} className="card-img-top img-fluid rounded-0" style={{ height: 200 }} alt="Drug" />
                            ...
                            <div className="row align-items-center text-center g-0">
                                ...
                                <div className="col-8">
                                    <Link to={`/view/${drug._id}`} className="btn btn-primary w-100 p-1 rounded-0">
                                        View
                                    </Link>
                                </div>
                            </div>
